# How to contribute to the MyWorkpapers/ui-toolkit project
...
3. Set this repo as the `upstream` repo `git remote add upstream git@github.com/myworkpapers/ui-toolkit.git`
4. Disallow direct pushing to upstream `git remote set-url --push upstream no_push`
5. Rebase by default to avoid merge bubbles `git config --global branch.autosetuprebase always`
...
3. `git flow feature publish {feature_name}` will push it back up to your fork on BitBucket.
...
5. Use BitBucket to raise a Pull Request. Add labels as appropriate. Then paste the url of the PR into the `#development` Slack channel with a request for someone to please review the changes. See below for how to name pull requests.
...
4. Now go back to BitBucket and raise a Pull Request to merge the upstream master from your fork's `master` branch. When that goes through you are done.
...
There are various systems, including BitBucket itself, which will pick up the issue numbers from commit messages and pull requests and automatically associate them with the issues. It is therefore desirable to use a formal naming scheme for features, commit messages and pull requests.
